...
if(isset($_POST['submit'])) {
	...
	$query ="UPDATE featured_project SET title='".$_POST['title']."', subtitle='".$_POST['subtitle']."' where id='".$id."'";
	...
	for($i=0; $i<count($_FILES['upload']['name']); $i++) 
	{
		...
		if ($tmpFilePath != "")
		{
			$newFilePath =  $_SERVER['DOCUMENT_ROOT'] .'/featuredProjectImage/'.$short_code1. $_FILES["upload"]['name'][$i];
			
			if(move_uploaded_file($tmpFilePath, $newFilePath)) 
			{
				if($_FILES['upload']['name'][0] != "")
				{
				$updateimgs = mysql_query("UPDATE featured_project set title='".$_POST['title']."', subtitle='".$_POST['subtitle']."', image = '".$short_code1.$_FILES['upload']['name'][0]."' where id='".$id."'");	
				}
			}
		}
	}
    ...
    window.location.href='featured_projects.php';
    </SCRIPT>");
}	
?>
<!DOCTYPE html>
                                ...
                                <div class="span12">
                                     <form class="form-horizontal" action='' method='post' enctype="multipart/form-data">
									  ...
									  <tr>
										 ...
										 <p> </p>
                                         <td> 
                                           <input type='file' name='upload[]'>  <span><img src='../featuredProjectImage/<?php echo $row['image']; ?>' style='height:45px; height:45px;'></span>
                                          </td>
